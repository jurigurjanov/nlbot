...
class TradingBot:
    ...
    def __init__(self, config: BotConfig):
        ...
        self._multi_strategy_rollout_mode_override_by_symbol = self._resolve_multi_strategy_rollout_mode_overrides()
        ...
        self._db_first_tick_poll_interval_sec = max(
            0.25,
            self._env_float("XTB_DB_FIRST_TICK_POLL_INTERVAL_SEC", 1.0),
        )
        ...
        self._db_first_tick_hard_max_age_sec = max(
            1.0,
            self._env_float("XTB_DB_FIRST_TICK_HARD_MAX_AGE_SEC", 10.0),
        )
        self._db_first_tick_prune_interval_sec = max(
            0.25,
            self._env_float("XTB_DB_FIRST_TICK_PRUNE_INTERVAL_SEC", 1.0),
        )
        ...
        self._db_first_symbol_spec_poll_interval_sec = max(
            1.0,
            self._env_float("XTB_DB_FIRST_SYMBOL_SPEC_POLL_INTERVAL_SEC", 6.0),
        )
        db_first_symbol_spec_refresh_age_sec = self._env_float(
            "XTB_DB_FIRST_SYMBOL_SPEC_REFRESH_AGE_SEC",
            self._env_float("XTB_DB_FIRST_SYMBOL_SPEC_MAX_AGE_SEC", 900.0),
        )
        ...
        self._db_first_tick_error_log_interval_sec = max(
            1.0,
            self._env_float("XTB_DB_FIRST_TICK_ERROR_LOG_INTERVAL_SEC", 20.0),
        )
        self._db_first_disconnect_error_log_interval_sec = max(
            5.0,
            self._env_float("XTB_DB_FIRST_DISCONNECT_ERROR_LOG_INTERVAL_SEC", 60.0),
        )
        self._db_first_reconnect_min_interval_sec = max(
            1.0,
            self._env_float("XTB_DB_FIRST_RECONNECT_MIN_INTERVAL_SEC", 5.0),
        )
    ...
    def _strategy_params_for(self, strategy_name: str) -> dict[str, object]:
        ...
        params = self.config.strategy_params_map.get(normalized)
        if params is None:
            if normalized == str(self.config.strategy).strip().lower():
    ...
    def _worker_assignment_payload(
        ...
        if normalized == _MULTI_STRATEGY_CARRIER_NAME:
            base_name = str(
                params.get(_MULTI_STRATEGY_BASE_COMPONENT_PARAM) or self.config.strategy
            ).strip().lower()
            return (
    ...
    def _strategy_supports_symbol(
        ...
        try:
            ...
            if normalized_strategy == _MULTI_STRATEGY_CARRIER_NAME:
                base_strategy_name = str(
                    strategy_params.get(_MULTI_STRATEGY_BASE_COMPONENT_PARAM) or self.config.strategy
                ).strip().lower()
                ...
                strategy_filter = create_strategy(strategy_name, strategy_params)
    ...
    def _schedule_disabled_by_multi_strategy(self) -> bool:
        params = self._strategy_params_for(self.config.strategy)
    ...
    def _resolve_multi_strategy_rollout_mode_overrides(self) -> dict[str, RunMode]:
        ...
        if stage in {"canary", "pilot"}:
            ratio = self._as_float_param(params.get("multi_strategy_rollout_canary_ratio"), 0.30)
            ratio = max(0.0, min(1.0, ratio))
            explicit_canary = {
                symbol
                for symbol in self._as_symbol_list_param(params.get("multi_strategy_rollout_canary_symbols"))
                if symbol in seen
            }
    ...
    def _db_first_tick_request_interval_sec(self) -> float:
        # One /markets request is made per loop iteration.
        # Bound call rate by both explicit poll interval and per-minute target budget.
        target_interval = 60.0 / max(1.0, self._db_first_tick_target_rpm)
        return max(
            0.25,
            float(self._db_first_tick_poll_interval_sec),
            float(self.config.ig_rest_market_min_interval_sec),
            target_interval,
        )
    ...
    def _db_first_tick_effective_hard_max_age_sec(self, active_symbol_count: int | None = None) -> float:
        ...
        # Keep at least one full active-symbol cycle in cache, with modest jitter headroom.
    ...
    def _db_first_tick_max_age_for_workers(self) -> float:
        ...
        # DB-first tick producer loops symbols serially; with many symbols and IG REST pacing
        # we need a max-age window that covers at least a full cache refresh cycle.
    ...
    def _estimated_active_db_first_symbol_count(self) -> int:
        ...
        symbols: set[str] = set()
        for symbol in self._schedule_assignments(now_utc).keys():
    ...
    def _update_latest_tick_from_broker(self, tick: PriceTick) -> None:
        ...
        if not math.isfinite(bid) or not math.isfinite(ask):
            return
        ts = self._normalize_timestamp_seconds(getattr(tick, "timestamp", time.time()))
        if not math.isfinite(ts) or ts <= 0:
            ts = time.time()
        received_at = self._normalize_timestamp_seconds(getattr(tick, "received_at", time.time()))
    ...
    def _symbols_for_db_first_symbol_spec_refresh(self, now_utc: datetime | None = None) -> list[str]:
        return self._runtime_symbols_for_db_first_requests(now_utc)
    ...
    def _db_first_tick_symbol_buckets(self) -> tuple[list[str], list[str]]:
        all_symbols = self._runtime_symbols_for_db_first_requests()
        if not all_symbols:
    ...
    def _db_first_tick_cache_loop(self) -> None:
        ...
        while not self.stop_event.is_set() and not self._db_first_cache_stop_event.is_set():
            ...
            if not active_symbols and not passive_symbols:
                self._db_first_cache_stop_event.wait(timeout=request_interval_sec)
                continue
            ...
            if backoff_remaining > 0.0:
                self._db_first_cache_stop_event.wait(timeout=max(0.5, min(backoff_remaining, request_interval_sec)))
                continue
            ...
            try:
                ...
                if tick is None:
                    self._db_first_cache_stop_event.wait(timeout=min(0.5, request_interval_sec))
                    continue
                ...
                self._db_first_cache_stop_event.wait(timeout=max(request_interval_sec, backoff_sec))
                continue
            self._maybe_prune_db_first_stale_ticks(active_symbol_count=len(active_symbols))
            self._db_first_cache_stop_event.wait(timeout=request_interval_sec)
    ...
    def _maybe_prune_db_first_stale_ticks(self, *, active_symbol_count: int | None = None) -> None:
        ...
        try:
            self.store.prune_stale_broker_ticks(
                max_age_sec=effective_hard_max_age_sec,
                now_ts=now,
            )
        except Exception:
            # Tick-prune should never stop cache producer loop.
            return
    ...
    def _maybe_record_db_first_tick_cache_refresh_error(self, symbol: str, error: Exception) -> None:
        ...
        allowance_related = self._is_allowance_related_error(error_text)
        reason = self._normalize_broker_error_reason(error_text)
        disconnected_reason = reason == "ig_api_client_is:failed"
        if disconnected_reason:
            self._maybe_reconnect_broker_for_db_first(now)
            ...
            self.store.record_event("WARN", None, message, payload)
            logger.warning(
        ...
        with self._workers_lock:
            assignment = self._worker_assignments.get(upper_symbol)
        payload: dict[str, object] = {
            "error": error_text,
            "reason": reason,
            "streak": streak,
        ...
        backoff_remaining_sec = self._broker_public_api_backoff_remaining_sec()
        ...
        self.store.record_event(level, upper_symbol, message, payload)
            ...
            try:
                ...
                if (
                    ...
                    self.store.record_event(
                        ...
                        {"error": str(exc), "mode": self.config.mode.value},
                    )
    ...
    def _db_first_symbol_spec_cache_loop(self) -> None:
        ...
        while not self.stop_event.is_set() and not self._db_first_cache_stop_event.is_set():
            ...
            symbols_to_refresh = [
                symbol
                for symbol in symbols
                if self.store.load_broker_symbol_spec(
                    symbol,
            ...
            if not symbols_to_refresh:
                ...
                self._db_first_cache_stop_event.wait(timeout=wait_timeout_sec)
                continue
            backoff_remaining = self._broker_public_api_backoff_remaining_sec()
            if backoff_remaining > 0.0:
                self._db_first_cache_stop_event.wait(timeout=max(0.5, min(backoff_remaining, request_interval_sec)))
    ...
    def _symbol_spec_preload_is_strict(self) -> bool:
        ...
        # Unknown wrappers should keep strict mode enabled by default.
        return True

    def _broker_candidate_chain(self, max_depth: int = 4) -> list[object]:
        chain: list[object] = []
        candidate: object = self.broker
    ...
    def _build_startup_symbol_spec_fallback(
        ...
        *,
        ...
        allowance_related = (
            self._is_allowance_related_error(normalized_error)
            or self._broker_public_api_backoff_remaining_sec() > 0.0
        )
        if (
            normalized_error
        ...
        for candidate in self._broker_candidate_chain():
            ...
            metadata["startup_fallback_reason"] = self._slug_reason(
                normalized_error,
    ...
    def _preload_symbol_specs_on_startup(self) -> None:
        ...
        for symbol in symbols:
            if self.stop_event.is_set():
                break
            cached_fresh = self.store.load_broker_symbol_spec(
                symbol,
                max_age_sec=self._db_first_symbol_spec_refresh_age_sec,
            )
            ...
            previous = self.store.load_broker_symbol_spec(symbol, max_age_sec=0.0)
            if allowance_short_circuit or self._broker_public_api_backoff_remaining_sec() > 0.0:
                if previous is not None:
                    loaded_cached_count += 1
                else:
            ...
            while not self.stop_event.is_set():
                if self._reserve_ig_non_trading_budget(
        ...
        self.store.record_event(level, None, "Startup symbol specification preload complete", payload)
        if failed:
