...
class TradingBot:
    ...
    def _run_monitor_task(self, task_name: str, operation: Callable[[], None]) -> None:
        self._worker_health._run_monitor_task(task_name, operation)

    def _pulse_runtime_monitor_progress(self) -> None:
        self._worker_health._pulse_runtime_monitor_progress()

    def _restart_worker_for_symbol(self, symbol: str, *, reason: str) -> bool:
        return self._worker_health._restart_worker_for_symbol(symbol, reason=reason)

    def _worker_health_check_once(self) -> list[str]:
        return self._worker_health._worker_health_check_once()

    def _worker_stale_heartbeat_check_once(self) -> list[str]:
        return self._worker_health._worker_stale_heartbeat_check_once()

    def _dump_runtime_thread_traces(self, *, reason: str) -> None:
        self._worker_health._dump_runtime_thread_traces(reason=reason)

    def _abort_process_for_watchdog(self, *, reason: str) -> None:
        self._worker_health._abort_process_for_watchdog(reason=reason)
